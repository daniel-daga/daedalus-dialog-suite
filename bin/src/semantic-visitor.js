...
exports.SemanticModelBuilderVisitor = void 0;
const semantic_model_1 = require("./semantic-model");
const action_parsers_1 = require("./action-parsers");
const condition_parsers_1 = require("./condition-parsers");
class SemanticModelBuilderVisitor {
    constructor() {
        this.semanticModel = { dialogs: {}, functions: {}, hasErrors: false, errors: [] };
        this.currentInstance = null;
        this.currentFunction = null;
        this.conditionFunctions = new Set();
    }
    /**
     * Check for syntax errors in the parse tree and populate semantic model errors
     */
    checkForSyntaxErrors(node, sourceCode) {
        if (node.hasError) {
     ...
     */
    pass1_createObjects(node) {
        if (node.type === 'function_declaration') {
            const nameNode = node.childForFieldName('name');
            const typeNode = node.childForFieldName('return_type');
            if (nameNode && typeNode) {
                const func = new semantic_model_1.DialogFunction(nameNode.text, typeNode.text);
            ...
            if (nameNode) {
                const dialog = new semantic_model_1.Dialog(nameNode.text, parentNode ? parentNode.text : null);
    ...
    processFunctionCall(node) {
        ...
        if (funcToCallNode && this.currentFunction) {
            ...
            if (isConditionFunc) {
                ...
                const condition = condition_parsers_1.ConditionParsers.parseSemanticCondition(node, functionName);
                if (condition) {
                ...
                const action = action_parsers_1.ActionParsers.parseSemanticAction(node, functionName);
                if (action) {
     ...
     */
    findDialogForFunction(functionName) {
        for (const dialogName in this.semanticModel.dialogs) {
            ...
            if (dialog.properties.information &&
                dialog.properties.information.name === functionName) {
                return dialog;
            }
        }
        return null;
     ...
     */
    findDialogForConditionFunction(functionName) {
        for (const dialogName in this.semanticModel.dialogs) {
            const dialog = this.semanticModel.dialogs[dialogName];
            if (dialog.properties.condition &&
                dialog.properties.condition.name === functionName) {
                return dialog;
            }
        }
        return null;
...
exports.SemanticModelBuilderVisitor = SemanticModelBuilderVisitor;
