...
/**
 * #278: a notes file groups its TOPIC_ constants under `//` headers
 * ("Lehrer", "Händler", "Sonstiges"). The in-game diary has no such level —
 * Log_CreateTopic knows only missions and notes — so the grouping is purely
 * where the declaration sits in the script.
 *
 * A header is a run of consecutive standalone `//` lines holding at least one
 * with text once decoration (`*`, `-`, `=`, …) is stripped. A commented-out
 * declaration is content, not a header.
 */
interface NoteSection { title: string; headerEnd: number; nextHeader: number }

const COMMENT_LINE = /^\s*\/\/(.*)$/;
const DECORATION = /^[\s*=\-#_~/+]+|[\s*=\-#_~/+]+$/g;
const COMMENTED_CODE = /^\s*(const|var|func|instance|prototype|class)\b|;\s*(\/\/.*)?$/i;

function findNoteSections(lines: string[]): NoteSection[] {
  ...
  while (i < lines.length) {
    ...
    let title = '';
    while (i < lines.length) {
      const match = lines[i].match(COMMENT_LINE);
      if (!match || COMMENTED_CODE.test(match[1])) break;
      title ||= match[1].replace(DECORATION, '');
      i++;
    }
    if (i === start) {
      i++;
    ...
    if (title) {
      if (sections.length) sections[sections.length - 1].nextHeader = start;
      sections.push({ title, headerEnd: i - 1, nextHeader: lines.length });
    }
  }
  return sections;
}

export function listNoteSections(content: string): string[] {
  return findNoteSections(content.split(/\r?\n/)).map((s) => s.title);
}

/** Insert `line` after the last non-blank line of the section titled `sectionTitle`. */
export function insertIntoNoteSection(content: string, sectionTitle: string, line: string): string {
  const eol = content.includes('\r\n') ? '\r\n' : '\n';
  const lines = content.split(/\r?\n/);
  const section = findNoteSections(lines).find((s) => s.title === sectionTitle);
  if (!section) {
    throw new Error(`No "${sectionTitle}" section found in the target file.`);
  }
  let insertAfter = section.headerEnd;
  for (let j = section.headerEnd + 1; j < section.nextHeader; j++) {
    if (lines[j].trim()) insertAfter = j;
  }
