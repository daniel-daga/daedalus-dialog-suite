...
import {
  Button, IconButton, MenuItem, Stack, TextField, ToggleButton, ToggleButtonGroup, Tooltip,
} from '@mui/material';
...
const WorldEditControls: React.FC<WorldEditControlsProps> = ({
  ...
  <>
    ...
    {/* Snapping's per-VOB half (level-editor.md §16.5) — unlike the gizmo,
        ...
        the selection whatever its size. */}
    {hasWorld && (
    ...
    {/* The whole selection: an append moves no index path, so the copies
        share one batch and one undo (D4). */}
    {hasWorld && (
      <Button
        size="small"
        variant="outlined"
      ...
      >
        {selectionCount > 1 ? `Duplicate ${selectionCount} VOBs` : 'Duplicate VOB'}
      </Button>
    )}
