...
describe('QuestLiteGraphCanvas condition expression capsule', () => {
  ...
  it('renders IF capsule even when dialog starts without a condition expression', () => {
    renderCanvas({
      nodes: [createDialogNode({ data: { ...createDialogNode().data, conditionExpression: '' } })]
    });

    expect(screen.getByRole('button', { name: /if:/i })).toBeInTheDocument();
    expect(screen.getByText(/No condition expression\./i)).toBeInTheDocument();
  });
