...
test.describe('External change conflict', () => {
  ...
  test('shows an on-disk diff of both versions in the conflict dialog', async ({ page }) => {
    await openDialogAndMakeDirty(page);
    await injectExternalChange(page);

    const diff = page.getByTestId('external-conflict-diff');
    await expect(diff).toBeVisible();

    // The disk side is the seeded file; the editor side is the code generated
    // from the (edited) model by the mock codegen — their first lines differ,
    // so the diff must show a removed disk line and an added editor line.
    const diffText = (await diff.textContent()) ?? '';
    expect(diffText).toContain('-// External conflict test file');
    expect(diffText).toContain('+// Generated by Daedalus Dialog Editor');

    // The resolution buttons stay available below the diff.
    await expect(page.getByTestId('external-conflict-keep-mine')).toBeVisible();
    await expect(page.getByTestId('external-conflict-reload')).toBeVisible();
  });
