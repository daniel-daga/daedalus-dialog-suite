...
**Active plan:** [`docs/plans/production-readiness-review-findings.md`](docs/plans/production-readiness-review-findings.md) — production-readiness / performance / UI-UX review, including the decision to deprecate the quest Flow view (Option A and Option B both landed: the litegraph Flow view has been removed; the quest surface is the read-only list/details/create panel). §3 Performance is closed except for its two remaining P3 measure-first items (the P0/P1/P2 items and the P3 bundle-size item all landed; durable outcomes in `docs/architecture/render-performance.md`), and the §5 post-release fast-follows are all landed too (F2 dead source-view cleanup, F6 Ctrl+F scoping, and a strict `default-src 'self'` CSP — which moved Monaco off the jsdelivr CDN to the app's own origin; see `docs/architecture/security-model.md`). §2 blocker 6 is fully closed as of 2026-08-24: the renderer now carries a six-boundary error-boundary map whose close-guard boundary fails safe by cancelling the close, and caught crashes are logged through the existing `logRendererError` channel (`docs/architecture/security-model.md` → *Renderer crash containment*). Its §5 tracks what has landed and what remains.
