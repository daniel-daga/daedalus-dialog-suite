...
| 10 | `window-close-guard.spec.ts` | Real OS window `close` on a dirty app → main vetoes and sends `app:closeRequested` → the renderer close-guard dialog appears → "Save and close" writes to disk and the window closes; "Cancel" keeps it open; the 3 s force-destroy fires when the renderer never ACKs. Also asserts **atomic write**: kill the write mid-flight and the target file keeps its original bytes (E5). Deferred here from slice 2 (fix-02 E1/E5) — the mock harness cannot exercise a real window close, atomic temp+rename, or the force-destroy timer; slice 2 ships only the renderer-dialog spec (`tests/e2e/close-guard.spec.ts`, event-injection) and the Jest close-guard/atomic-write suites. | slice 2 (E1 close guard, E5 atomic write) |
