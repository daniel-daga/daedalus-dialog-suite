...
| 7 | **CODE LANDED (2026-07-04)** — Electron 29 → 43 (bundled Node 20 → 24), electron-builder 24.13 → 26, `@types/node` → ^24, drop `build.electronVersion` pin (§2.7) | L | best after 1–5 (don't debug a new runtime and new security code simultaneously); slice 3 (worker reliability) ideally first since workers are the top ABI-risk surface | electron-builder 26 config re-validated against its v26 schema (all keys valid, none deprecated); NSIS `*.exe` glob and asar layout in `build-windows.yml` unchanged under v26 → workflow untouched. Local Node gate green (Jest 131/893, typecheck, main+renderer build, lint). NAPI `npmRebuild: false` invariant documented in `docs/architecture/worker-reliability.md`. **Still CI-/maintainer-verified**: `editor-e2e-electron` real-Electron suite under Electron 43 (the NAPI/worker safety net), packaged Windows smoke + real parse, and the packaging dispatch itself (blocked from local run — Electron/electron-builder binaries can't download in-sandbox). |
