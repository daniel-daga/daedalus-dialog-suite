...
`failureTarget` on `zCCodeMaster`. `zCTriggerUntouch` therefore has a catalogue
...
for both. `zCMoverController` is the whole class — `target` (the `vobName` of
the mover it drives, so the dangling warning and the name suggestions cover it
by key), `message` (a fifth entry in `CLASS_ENUM_FIELDS`, four values, written
unconditionally rather than under `zCMover`'s keyframe guard) and `key`.
`zCVobLensFlare` is its one string, `fx`. Neither is constructible by
`insertVob`, so each round-trips against a new fixture VOB (`1/21`, `1/22`)
rather than a placed one, and neither joined `AUTHORABLE_VOB_CLASSES` —
placing one is that set's question. `zCEarthquake` stayed out: its `amplitude`
is a float triple, and the catalogue's array kinds are `color` (four integers)
and `vec2` (two floats), so a third array kind is a decision of its own.
...
**What is left readable-and-uncatalogued**, after #260 took the two worth
taking: `oCNpc` and the cutscene camera pair (`zCCSCamera`,
`zCCamTrj_KeyFrame`), which are their own projects; `zCEarthquake`, held by the
float-triple kind above; the three `…Default` zone variants, deliberately absent
because a `zCZoneZFogDefault` is a world's fallback settings rather than a placed
zone; `oCCSTrigger`, which declares nothing beyond `VTrigger` and whose retail
prevalence nobody has counted; and `zCVobLevelCompo`, `zCVobStair` and
`zCVobScreenFX`, which declare no field of their own at all.
...
the gap and left it open; §16.18 added two more behind it; the Phase 1c overlay added a fourth
(§8) and argued that a fifth checker would only deepen the debt.
...
4. **Waypoint occupancy.** How many NPCs retail spawns on one waypoint, over
   the spawn index Phase 1c built (§8). The distribution's tail is the threshold.
...
pattern the duplicate-spawn rule met as 598 site pairs (§8). Only the distinct-instance column
...
recorded at §8, where the index it qualifies is described.
...
number §7 and §8 measured, so it is the same corpus those used; it is
not the shipped `.DAT`.
...
Four of the "five looks" (board, §8/§16.26/§16.12's landed features) came
...
| 18 | The OU database: the reader against a real `OU.BIN` (a project whose scripts match its OUs reports nothing), then *Update OUs* against a copy of one — read back, and the new subtitle shown in game. Reader and writer have only met hand-authored fixtures built from `CutsceneLibrary::load`'s read order | 2026-09-26 | #264; `spacer-gap-triage.md` §A1 |
