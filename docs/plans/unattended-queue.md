...
|---|---|---|---|---|
| 1 | `getSemanticModel` writes a stale model over a fresh one — no generation guard, and it caches files that are open in `fileStore` | 2026-07 **4.1**, `projectStore.ts:632` | `projectStore.staleCacheWrite` › a resolved stale parse does not land | M |
