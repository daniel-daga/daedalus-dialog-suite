...
top-down, so an unattended session needs one issue instead of forty.
...
|---|---|---|---|---|
| ~~40~~ | ~~**(corpus)** Re-measure spawn-index coverage~~ — **landed 2026-08-30**: 3,976 of 3,978, and the two missed are `Externals.d`'s prototype declarations. `countSpawnCalls` now strips comments, so the old 4,087 denominator counted 111 commented-out calls as lost ones | `docs/architecture/level-editor.md` §8 | `check-spawn-occupancy.js` reports the new coverage; the overlay's measurement paragraph and the occupancy table are updated with it | S |
...
| 42 | **blocked — issue #227, labelled `triage`; a run may not take it.** There is no BINARY `.zen` on this machine to classify (the whole install was enumerated); §14.3 3.1 carries the enumeration and the pieces it decomposes into. **The walker half closed 2026-09-12** (Daniel's cut): `lib/container-binary.js` exists, so a BINARY table would no longer read `struct-only` on every row — what is still missing is worlds to put in it. **(corpus)** The BINARY fidelity baseline: `extract-worlds.js` already takes directories, so point it at the install's loose `_work/Data/Worlds` and classify — BINARY has had no fidelity work at all | §14.3 3.1 | a per-world classification table; each defect it names then lands fixture-backed, as `0045`–`0048` did | M |
...
| ~~35~~ | ~~The a11y mechanical half: no `autoFocus`, no `aria-describedby`, clickable `Box` headers without button semantics~~ — **landed 2026-09-03**: the close guard, the conflict dialog and the delete confirm are `aria-describedby` their body text; the close guard focuses Save-and-close (Cancel while a conflict disables it) by the `DeleteConfirmDialog` ref-and-timeout idiom; the conflict dialog focuses nothing on purpose — both its resolutions lose something. The section headers' `IconButton` was already the focusable, Enter-able control, so it gained `aria-expanded` rather than the `Box` gaining a nested `role="button"`. The `html { fontSize: 80% }` question stays a decision | production-readiness §4 **F19-F22** | e2e `close-guard` › the close guard focuses its default action; `a11yMechanics` | M |
| ~~36~~ | ~~No Alt+Up/Down action reorder; `moveAction` is already a prop and the e2e harness exists~~ — **landed 2026-09-03**: Alt+Up/Down in `ActionCard.handleKeyDown` calls `moveAction` on the card's own list, bounded so a branch's last line does not escape into its parent, and re-focuses the moved card on the next tick. The Tab half of 5.4 stays a decision | 2026-07 **5.4** (the Alt half only) | `keyboard-dnd-reorder` -g "Alt+Arrow" | M |
| ~~37~~ | ~~"View release notes" is a dead `href="#"`; there is no `shell.openExternal` channel~~ — **landed 2026-09-03**: `shell:openExternal` + `assertExternalUrl` (https only — `file:`, `javascript:`, a bare host all refused) + `editorAPI.openExternal`; the link carries the release URL as its `href` and opens it through the channel | production-readiness §2, `UpdateNotification.tsx:169` | `ipcValidation` › rejects a non-https URL; `UpdateNotification.releaseNotes` › the link opens the release URL | M |
...
| ~~39~~ | ~~The packaged-app asar check asserts the ZenKit addon but **not** the tree-sitter binding, and no packaged smoke parses a `.d`~~ — **landed 2026-09-03**: `parseSmoke.ts` + `DDE_SMOKE_PARSE`, the open-world smoke's twin, driving `ParserService.parseSource` over `corpus/declaration-order.d` (1 dialog, 2 functions). `asarUnpack` widened to `daedalus-parser` and `tree-sitter` — both bindings *were* packaged but packed **inside** the asar, loading only through Electron's dlopen-extract fallback — and the asar check now asserts each is present **and** unpacked, `tree-sitter`'s as its `win32-x64` prebuild. Verified against a real local package, both ways: the check and the smoke pass on it, and each goes red when its binding is hidden. **Forward:** with the grammar binding gone the packaged app *hangs* rather than exiting 1 — no result file, no output (a GUI-subsystem exe redirects none) — so that failure reaches CI as the step's 120 s timeout, not as a message. Diagnosed no further; a red parse smoke that times out means "binding missing", not "CI slow" | production-readiness §2 blocker 3, `build-windows.yml:153` | the smoke entry's Jest test and the asar assertion; the workflow step itself needs a dispatch | L |
