...
**The ops this gate covers are the five named above, and no others.** Four
have shipped since the candidate was built and therefore have **no engine
verdict at all**: `DeleteVob`, `MoveWaypoint`, `SetVobClassProp` and
`RenameWaypoint`. Quote this
...
- **`05`'s two observation rows** — closed by the third pass, below.
...
### Gate 2b, third pass — 2026-08-30, `07`, the last two observation rows

`05`'s two rows, each in a frame built to show it and nothing else (run sheet
§07). Through GMBT, staged as `NEWWORLD.ZEN`, with the control in-session and
every candidate's staged hash confirmed in `results.log` before its verdict.
A first attempt the night before was void — the worlds ran under their own
file names, so no `STARTUP_` script spawned an NPC, and the lights-only
clearing left 22 other torches burning (environment-hazards.md, *GMBT*).

| Row | Result |
|---|---|
| `00` control | OK. Access violation only after quitting — the known shutdown crash |
| **`07a` — one torch, alone** | **PASS.** The frame cleared of 231 light/sound/effect VOBs and of 22 other wall-torch subtrees; the test torch stands, burning, the only lit thing. 22 subtree deletes witnessed as bare brackets |
| **`07b` — no torch** | **PASS.** The same frame and one `DeleteVob` on the torch's six-VOB subtree: post, flame, sparks, flare and lights all gone, nothing left floating. **The subtree delete is witnessed wholly, not partially** |
| **`07c` — the renumber alone** | **PASS.** `NW_XARDAS_TOWER_IN1_32` removed, 2,895 waypoints renumbered, no VOB touched; the tower's routines run on their waypoints |

With this, every op the editor ships has been seen doing its work in the
engine, not only loading. What is unwitnessed is the list above this section:
the decal fields, the enum writes, `oCZoneMusic.volume` by decision, and the
authorable classes beyond the five seen.
