...
### Upstreamable as-is — plain ZenKit bugs any consumer wants (21)
...
`0027` and `0029` are the same class of standalone fix — a one-guard hardening of the
read path, reachable by any consumer that opens a file it did not write. `0029` is
the stronger of the two: the bug it stops is an out-of-bounds write, not a hang.
...
1. `0020`, `0021`, `0022`, `0027`, `0029` — one PR each.
